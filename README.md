...
AI models can write scripts and automate workflows, but running them directly on your system is risky. Orbin solves this by providing:
...
Orbin is self-contained, lightweight, and designed to plug into AI workflows seamlessly.
...
```bash
docker run -d --name orbin \
  -p 8000:8000 \
  -v orbin-workspace:/workspace \
  -e ORBIN_API_KEY=your-key \
  orbin/runtime
```

Visit `http://localhost:8000`, your terminal is ready.

> Tip: If you don’t provide an API key, Orbin generates one automatically. Retrieve it with `docker logs orbin`.
...
```bash
orbin run --host 0.0.0.0 --port 8000 --api-key your-key
```
...
Integration can be **direct** (AI talks to your Orbin instance) or **proxied** (through a central server for multi-user setups).
